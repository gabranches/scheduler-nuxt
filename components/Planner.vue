<template>
  <div>
    <div class="row planner-row" v-bind:key="slot.dateStampRoutine" v-for="slot in scheduleTidy">
      <div class="col-2 planner-left">
        <div class="row slot-row">
          <div class="col">
            Date: <br>
            <select v-model="slot.dateStamp" v-on:change="editSlot(slot)">
              <option v-for="date in upcomingDates"
                v-bind:key="date.id"
                v-bind:value="date.dateStamp">{{ date.dateText }}</option>
            </select>
          </div>
        </div>
        <div class="row slot-row">
              ...
              <div class="col">
                ...
                <select v-model="slot.location" v-on:change="editSlot(slot)">
                  <option v-for="location in locations"
                    ...
                    v-bind:value="location.tag">{{ location.name }}</option>
                </select>
              </div>
            </div>
          </div>
        ...
        <div class="row slot-row">
          <div class="col">
            <button class="btn btn-danger"
              v-if="slot.edited"
              v-on:click="submitScheduleChange(slot)">Save Changes</button>
            <button class="btn btn-warning" disabled
              v-if="slot.isRoutine && !slot.edited">Routine Schedule</button>
          </div>
        </div>
      </div>
      <div class="col-10 planner-mid">
        <div class="row">
          <div class="col appt-col text-right" v-bind:key="timeSlot.start"
            ...
            <div class="row slot-row">
              ...
              <div class="col">
                <select v-model="timeSlot.type" v-on:change="editSlot(slot)">
                  <option v-for="type in slotTypes"
                    v-bind:key="type.type"
                    v-bind:value="type.type">{{ type.shortText }}</option>
                </select>
              </div>
            </div>
            <div class="row slot-row">
              ...
              <div class="col">
                <select v-model="timeSlot.start" v-on:change="editSlot(slot)">
                  <option v-for="start in scheduleSlots"
                    v-bind:key="start.time"
                    v-bind:value="start.time">{{ start.text }}</option>
                </select>
              </div>
            </div>
            <div class="row slot-row">
            ...
            <div class="row slot-row">
              ...
              <div class="col" v-bind:class="classBooked(timeSlot.booked)">
                {{ timeSlot.booked }}
              </div>
            </div>
          </div>
  ...
  methods: {
    ...
    async submitScheduleChange(scheduleSlot) {
      ...
      try {
        ...
        await axios.post(`${process.env.HOST_URL}/api/add/schedule-change`, scheduleSlot);
